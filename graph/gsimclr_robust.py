...
class simclr(nn.Module):
  ...
  def loss_cal(self, x, x_aug, mode, args):
    T = args.temp
    batch_size, _ = x.size()
    x_abs = x.norm(dim=1)
    ...
    if mode == "reweight_only":
        ...
        neg_sim = sim_matrix.masked_select(mask).view(batch_size, -1) / T
        pos_sim = sim_matrix[range(batch_size), range(batch_size)] / T
        loss_1 = - torch.mean(pos_sim)
        mu = args.w1
        sigma = args.w2
        weight = 1. / (sigma * math.sqrt(2 * math.pi)) * torch.exp( - (neg_sim - mu) ** 2 / (2 * math.pow(sigma, 2)))
        weight = weight / weight.mean(dim=-1, keepdim=True)
        loss_2 = torch.mean(torch.mean(neg_sim * weight.detach(), dim=1))
...
if __name__ == '__main__':
    ...
    dataset = TUDataset(path, name=DS, aug=args.aug, noise_ratio=args.noise_ratio).shuffle()
    ...
    print('{},{},{},{},{},{},{}'.format(args.DS, args.num_gc_layers, epochs, log_interval, lr, args.mode, args.t2))
    print('================')
    print(vars(args))
    model.eval()
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in dataloader:
            ...
            # print(x)
            # print(x_aug)
            loss = model.loss_cal(x, x_aug, args.mode, args)
            # print(loss)
    ...
    model_log_file=os.path.join(args.output_dir, args.model_name)
    with open(model_log_file, 'a') as f:
        str_output = "test\t{}\t{}\n".format(accuracies['test'][-2], accuracies['test'][-1])
        f.write(str_output)
    tpe  = ('local' if args.local else '') + ('prior' if args.prior else '')
    with open('logs/log_' + args.DS + '_' + args.aug, 'a+') as f:
        s = json.dumps(accuracies)
        f.write('{},{},{},{},{},{},{},{},{}\n'.format(args.DS, tpe, args.num_gc_layers, epochs, log_interval, lr, args.mode, args.t2, s))
        f.write('\n')
