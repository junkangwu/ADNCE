...
do
    ...
    do
        cd ..
        model_name="${dataset}_noise_${noise_ratio}_seed_${seed}_${aug}_mode_${mode}_t_${temp}_w1_${w1}_w2_${w2}"
        echo $model_name
        ...
        --temp $temp --w1 $w1 --w2 $w2 --model_name $model_name --noise_ratio $noise_ratio
        cd bash
    done
    if [[ $char_name == "w1" ]]
    then
