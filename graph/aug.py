...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
                 ...
                 cleaned=False, aug=None, noise_ratio=0.1):
        self.name = name
    ...
    def process(self):
        read_data_res = read_tu_data(self.raw_dir, self.name)
        if len(read_data_res) == 2:
            self.data, self.slices = read_data_res
        else:
            self.data, self.slices, _ = read_data_res
    ...
    def get(self, idx):
        ...
        if self.aug == 'dnodes':
            ...
            if n == 0:
               data_aug = drop_nodes(deepcopy(data), self.noise_ratio)
            elif n == 1:
               data_aug = subgraph(deepcopy(data), self.noise_ratio)
            else:
                print('sample error')
                assert False
...
def drop_nodes(data, noise_ratio=0.1):
    ...
    drop_num = int(node_num * noise_ratio)
...
def subgraph(data, noise_ratio=0.2):
    ...
    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
    sub_num = int(node_num * noise_ratio)
